...
def update_graphs(selected_scenario, selected_time_spent, username):
    ...
    star_scores = extract_scores("Levels")
    #print(star_scores)
    star_data = []
    for folder, levels in star_scores.items():
        for level, stars in levels.items():
            star_data.append({'Folder': folder, 'Level': level, 'Stars': stars})

    df_stars = pd.DataFrame(star_data)
    fig_stars = px.bar(df_stars, x='Level', y='Stars', color='Folder', title='Nombre d\'étoiles par niveau', text='Stars')
    fig_stars.update_traces(texttemplate='%{text}', textposition='outside')
    ...
    return fig_avg_score, fig_completed_counts, fig_time_spent, fig_stars
